...
A lightweight Chrome extension (Manifest V3) engineered for traders and investors to monitor live Gold (XAU/USD), Bitcoin (BTC/USD), Ethereum, and Forex without keeping heavy exchange tabs or desktop terminals open.
...
2. **Background Polling with Zero CPU Waste**: Uses `chrome.alarms` to fetch prices in the service worker at configurable intervals (1 min, 5 min) without running persistent background page loops.
3. **Multi-Asset Watchlist**: Tracks Gold, Bitcoin, Ethereum, Solana, and Forex with live 24h high/low and price change badges.
...
│  - Evaluates threshold alerts -> chrome.notifications  │
...
| **Ethereum (ETH)** | Binance `ETHUSDT` | CoinGecko API |
| **Solana (SOL)** | Binance `SOLUSDT` | CoinGecko API |
...
- [ ] **Phase 5: Sound / Audio Alert** — Subtle chime sound when a target price level is hit.
